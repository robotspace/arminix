...
/* as long as there is no bootloader, the original table cannot be used. This 
altered table has 2 major changes:
- Tasks that are normally loaded by the loader are replaced with undef_task() found in
  arch/ARM/exceptions.c
- The MEM and VM process are removed (NR_BOOT_PROCS is also changed).
*/
extern void undef_task(void);
...
{SYSTEM, sys_task,TSK_F,  8, TASK_Q, TSK_S, TSK_T,     0, no_c,"system"},
{HARDWARE,      0,TSK_F,  8, TASK_Q, HRD_S,     0,     0, no_c,"kernel"},
{PM_PROC_NR,    undef_task,SRV_F, 32,      3, 0,     SRV_T, SRV_M, c(pm_c),"pm"    },
{FS_PROC_NR,    undef_task,SRV_F, 32,      4, 0,     SRV_T, SRV_M, c(fs_c),"vfs"   },
{RS_PROC_NR,    undef_task,SRV_F,  4,      3, 0,     SRV_T, SYS_M, c(rs_c),"rs"    },
{DS_PROC_NR,    undef_task,SRV_F,  4,      3, 0,     SRV_T, SYS_M, c(ds_c),"ds"    },
{TTY_PROC_NR,   undef_task,SRV_F,  4,      1, 0,     SRV_T, SYS_M,c(tty_c),"tty"   },
//{MEM_PROC_NR,   0,SRV_F,  4,      2, 0,     SRV_T, SYS_M,c(mem_c),"memory"},
{LOG_PROC_NR,   undef_task,SRV_F,  4,      2, 0,     SRV_T, SYS_M,c(drv_c),"log"   },
{MFS_PROC_NR,   undef_task,SRV_F, 32,      4, 0,     SRV_T, SRV_M, c(fs_c),"mfs"   },
//{VM_PROC_NR,    0,SRV_F, 32,      3, 0,     SRV_T, SRV_M, c(vm_c),"vm"    },
{INIT_PROC_NR,  undef_task,USR_F,  8, USER_Q, 0,     USR_T, USR_M, no_c,"init"  },
};
